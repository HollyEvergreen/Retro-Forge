...
public class Shader
{
	...
	/// 1 => fragment
	/// </param>
	public unsafe Shader(string? path)
	{
		var shader = File.ReadAllBytes(path!)!;
		try
		{
			fixed(int* id = &ID){
				fixed(void* off = &shader[0])
				{
					GL.ShaderBinary(1, id, ShaderBinaryFormat.ShaderBinaryFormatSpirV, off, shader.Length);
				}
			}
		} catch (IndexOutOfRangeException err)
		{
			Logger.Error($"the shader is {shader.Length} bytes long");
		}
	}
	...
	public static List<string> CompileShaders(List<string> shaderCompilationParams)
	{
		...
		for (var i = 0; i < n; i++)
		{
			...
			//File.WriteAllBytes($"shaders/{outPath}", shader_code.ToArray());
			var file = File.Open($"shaders/{outPath}", FileMode.Open, FileAccess.Write, FileShare.ReadWrite);
	...
	private static Span<byte> CompileShader(string[] path)
	{
        ...
        unsafe
		{
			...
			Logger.Log($"bytecode size = {bytecodeLength}");
